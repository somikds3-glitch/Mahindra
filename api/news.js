// api/news.js - Vercel serverless function: fetch news from NewsData.io, dedupe, return JSON
// IMPORTANT: Do NOT hardcode your NEWS_API_KEY here. Add NEWS_API_KEY in Vercel Project Settings -> Environment Variables.
...
function jsonResponse(res, obj, status = 200) {
  // set CORS and JSON headers and send
...
export default async function handler(req, res) {
  ...
  const pagesPerCompany = Math.max(1, Math.min(6, parseInt(body.pagesPerCompany || 2, 10) || 2)); // cap at 6
  ...
  try {
    // sequential fetch (gentle on free API)
